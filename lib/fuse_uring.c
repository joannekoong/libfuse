					...
					const unsigned int commit_id)
{
	...
	if (sqe->cmd_op == FUSE_IO_URING_CMD_REGISTER) {
		if (fuse_uring_use_bufpool(queue)) {
			req->flags |= FUSE_URING_BUFPOOL;
			req->init.buf_size = queue->ring_pool->se->bufsize;
			req->init.nr_bufs = queue->ring_pool->queue_depth + 1;
		}
		if (fuse_uring_use_registered_buffers(queue))
			req->flags |= FUSE_URING_REGISTERED_BUFFERS;

		if (fuse_uring_use_zero_copy(queue)) {
			req->flags |= FUSE_URING_ZERO_COPY;
			req->init.zero_copy_index = ent->zero_copy_index;
		}
	}
}
		       ...
		       __u32 cmd_op)
{
	...
	if (fuse_uring_use_registered_buffers(req->ring_queue)) {
		sqe->buf_index = req->ring_queue->registered_buffers_index;
...
static int fuse_uring_bufpool_setup(struct fuse_ring_queue *queue,
				    size_t depth)
{
	...
	if (fuse_uring_use_registered_buffers(queue)) {
		...
		if (fuse_uring_use_zero_copy(queue)) {
			err = io_uring_register_buffers_sparse(ring, depth + 1);
			if (err)
				goto cleanup;
				     ...
				     size_t depth, int fd, int evfd)
{
	...
	if (fuse_uring_use_bufpool(queue)) {
		rc = fuse_uring_bufpool_setup(queue, depth);
		if (rc) {
		    fuse_log(FUSE_LOG_ERR,
			     "Failed to setup bufpool for ring idx %zu: %s\n",
			     qid, strerror(rc));
