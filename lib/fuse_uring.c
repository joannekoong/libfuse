					...
					const unsigned int commit_id)
{
	...
	if (sqe->cmd_op == FUSE_IO_URING_CMD_REGISTER) {
		if (fuse_uring_use_bufring(queue))
			req->flags |= (FUSE_URING_BUF_RING | FUSE_URING_PINNED_HEADERS |
		...
		req->init.buf_size = queue->ring_pool->se->bufsize;
	}
}
				    ...
				    size_t depth)
{
	...
	unsigned payload_size = queue->ring_pool->se->bufsize;
	int err;

	queue->payload_size = payload_size * depth;
	queue->payload_addr =
		mmap(NULL, queue->payload_size, PROT_READ | PROT_WRITE,
				  MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	if (!queue->payload_addr)
		goto cleanup;

	queue->headers = mmap(NULL, headers_len, PROT_READ | PROT_WRITE,
			      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	if (!queue->headers)
		goto cleanup;
	queue->headers_size = headers_len;
...
static void fuse_session_destruct_uring(struct fuse_ring_pool *fuse_ring)
{
	for (size_t qid = 0; qid < fuse_ring->nr_queues; qid++) {
		...
		if (!fuse_uring_use_bufring(queue)) {
			...
			munmap(queue->headers, queue->headers_size);
...
static int fuse_uring_prepare_fetch_sqes(struct fuse_ring_queue *queue)
{
	...
	for (size_t idx = 0; idx < ring_pool->queue_depth; idx++) {
		...
		ent->iov[0].iov_base = queue->headers;
		ent->iov[0].iov_len = queue->headers_size;

		ent->iov[1].iov_base = queue->payload_addr;
		ent->iov[1].iov_len = queue->payload_size;
				  ...
				  struct io_uring_cqe *cqe)
{
	...
	if (fuse_uring_use_bufring(queue)) {
		...
		buf_id = ent_in_out->buf_id;
			payload_size = fuse_ring->se->bufsize;
		ent->op_payload = queue->payload_addr + (buf_id * payload_size);
	}
